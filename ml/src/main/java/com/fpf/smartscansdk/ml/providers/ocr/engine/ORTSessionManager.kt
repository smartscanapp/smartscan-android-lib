...
class ORTSessionManager(
    ...
    suspend fun loadModels(detAssetSource: ModelAssetSource, recAssetSource: ModelAssetSource) {
        ...
        opts.use { opts ->
            ...
            val detBytes = readModelAsset(detAssetSource)
            val recBytes = readModelAsset(recAssetSource)
            try {
    ...
    private suspend fun readModelAsset(assetSource: ModelAssetSource): ByteArray {
        return try {
            when(assetSource){
                is ModelAssetSource.Resource -> ResourceOnnxLoader(context.resources, assetSource.resId).load()
                is ModelAssetSource.LocalFile -> FileOnnxLoader(assetSource.file).load()
            }
        } catch (t: Throwable) {
            throw OCRError.ModelNotFound(assetSource, t)
        }
    }
