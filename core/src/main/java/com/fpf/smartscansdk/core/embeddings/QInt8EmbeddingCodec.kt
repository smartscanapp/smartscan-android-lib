...
internal class QInt8EmbeddingCodec(
    private val embeddingDimension: Int,
    private val headerSize: Int,
): EmbeddingCodec {

    // Long + Long + ByteArray
    private val recordSize = 8 + 8 + embeddingDimension
    ...
    override suspend fun append(file: File, embeddings: List<StoredEmbedding>, idToFileOffsetIndex: MutableMap<Long, Long>): Int = withContext(Dispatchers.IO) {
        RandomAccessFile(file, "rw").use { raf ->
            ...
            val existingCount = if (fileExistsAndHasContent) readHeader(channel) else 0
            val newCount = existingCount + embeddings.size
            // Move to the end (append mode) or start of data section if new file
            val nextOffset = if (fileExistsAndHasContent) channel.size() else headerSize.toLong()
            ...
            for (embedding in embeddings) {
                ...
                for (byte in embedding.embedding.vector) writeBuffer.put(byte)
            }
    ...
    override suspend fun update(
        ...
        RandomAccessFile(file, "rw").use { raf ->
            ...
            for (emb in embeddings) {
                ...
                for (byte in emb.embedding.vector) buf.put(byte)
                buf.flip()
