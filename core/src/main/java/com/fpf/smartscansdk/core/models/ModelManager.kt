...
object ModelManager {
    ...
    suspend fun downloadModelInternal(context: Context, modelInfo: ModelInfo, onProgress: ((progress: Int) -> Unit)? = null): Result<File> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (isZipTarget) {
                ...
                val downloadResult = downloadFileInternal(modelInfo.url, tempZipFile, onProgress)
    ...
    suspend fun importModel(context: Context, modelInfo: ModelInfo, uri: Uri) = withContext(Dispatchers.IO){
        val targetFileOrDir = getModelFile(context, modelInfo)
        val isZipTarget = !modelInfo.resourceFiles.isNullOrEmpty()
        ...
        if (isZipTarget) {
            ...
            targetFileOrDir.parentFile?.mkdirs()
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                FileOutputStream(targetFileOrDir).use { outputStream -> inputStream.copyTo(outputStream) }
            }
        }
    }
    ...
    private suspend fun unzipFiles(zipFile: File, targetDir: File): List<File> = withContext(Dispatchers.IO) {
    ...
    private suspend fun downloadFileInternal(url: String, outputFile: File, onProgress: ((progress: Int) -> Unit)? = null): Result<File> = withContext(Dispatchers.IO) {
